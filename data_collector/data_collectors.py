import sys, os
from .census_data_collector.censusdc.utils import (
    CensusTimeSeries,
    create_filter,
    TigerWebMapServer,
)
...
class DataCollector(object):
    ...
    def get_training_data_for_shapefile(
        self, thread_num=1, years=None, climateFilter=None
    ):
        ...
        if 1:  ## census data
            cfilter = None
            ts = CensusTimeSeries(
                shp, self.apikey, field=self.fieldName.lower(), filter=cfilter
            )
            fidw = open(
                os.path.join(
                    self.output_folder, "failed_pid_{}.dat".format("None")
                ),
                "w",
            )
            ...
            for ipid, pid in enumerate(dfS[self.fieldName.lower()].values):
                try:
                    print(
                        "====== Downloading {}. Total Downloaded is {}".format(
                            pid,
                            int(
                                100
                                * float(ipid)
                                / len(dfS[self.fieldName.lower()].values)
                            ),
                        )
                    )
                    ...
                    df2 = CensusTimeSeries.interpolate(
                        df,
                        skip_years=range(2005, 2010),
                    ...
                    df2.to_csv(
                        os.path.join(
                            self.output_folder, "cs_{}.csv".format(pid)
                        ),
                        index=False,
                    )
                except:
    ...
    def get_training_data(
        self, huc2=None, thread_num=1, years=None, climateFilter=None
    ):
        """
        ...
        if 1:
            ...
            ts = CensusTimeSeries(
                shp, self.apikey, field=self.fieldName, filter=cfilter
            )
            fidw = open(
                os.path.join(
                    self.output_folder, "failed_pid_{}.dat".format(huc2)
                ),
                "w",
            )
            avail_years = [year for year in TigerWebMapServer.base.keys()]
            ...
            for ipid, pid in enumerate(cfilter):
                try:
                    print(
                        "====== Downloading {}. Total Downloaded is {}".format(
                            pid, int(100 * float(ipid) / len(cfilter))
                        )
                    )
    ...
    def get_prediction_data(
        self, huc2=13, apikey="", thread_num=8, years=[2000, 2001]
    ):
        pass
