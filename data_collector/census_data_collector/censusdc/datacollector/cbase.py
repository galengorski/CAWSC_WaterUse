...
class CensusBase(object):
                ...
                for key in (TigerWebVariables.state,):
                    if key in feature.properties:
                        ...
                        raise AssertionError(
                            "State number must be in " "properties dict"
                        )
                if tmp == 1:
                    for key in (
    ...
    def get_data(
        ...
        if level == "finest":
            ...
            if (
                self.__ilevel_dict[level]
                > self.__ilevel_dict[self.features_level]
            ):
                raise AssertionError(
                    "Cannot grab level data finer than {}".format(
                        self.features_level
                    )
                )
        ...
        if lut is None:
            raise KeyError(
                "No {} server could be found for {} and {}".format(
                    self._text, self.year, level
                )
            )
        ...
        if multiproc:
            ...
            twv = {
                k: v
                for k, v in TigerWebVariables.__dict__.items()
                ...
                if out is None:
                    ...
                    for dix, header in enumerate(data[0]):
                        if header == "NAME":
                            ...
                            try:
                                self._features[name][featix].properties[
                                    header
                                ] = float(data[1][dix])
    ...
    def __request_data(
        self, feature, featix, name, level, fmt, variables, url, retry, verbose
    ):
        """
        ...
        if level == "block_group":
            ...
            loc = fmt.format(feature.properties[TigerWebVariables.state])
        else:
        ...
        if self.year == 1990:
            payload = {"get": variables, "for": loc, "key": self.__apikey}
        else:
            payload = {
                "get": "NAME," + variables,
                "for": loc,
    ...
    def threaded_request_data(
        ...
        self.__request_data(
            feature, featix, name, level, fmt, variables, url, retry, verbose
        )
...
def multiproc_data_request(
    ...
    if year == 1990:
        payload = {"get": variables, "for": loc, "key": apikey}
    else:
        payload = {"get": "NAME," + variables, "for": loc, "key": apikey}
    ...
    if verbose:
        print(
            "Getting {} data for {} feature # {}".format(level, name, featix)
        )
    try:
        data = r.json()
    except JSONDecodeError:
        ...
        if verbose:
            print(
                "Error getting {} data for {} feature # {}".format(
                    level, name, featix
                )
            )
    if len(data) == 2:
