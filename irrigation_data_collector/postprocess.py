                ...
                if filename.endswith(".csv"):
                    ...
                    if var_name in var:
                        ...
                        if (shape == "idaho") or (shape == "mississippi"):
                            ...
                            id_name = os.path.splitext(filename)[0].split("_")[
                                1
                            ]
    ...
    for year in year_list:
        ...
        tmmx_k = pd.read_csv(
            os.path.join(
                ...
                shape,
                year,
                shape + "_" + "tmmx_k" + "_" + year + ".csv",
            )
        )
        sph_kgkg = pd.read_csv(
            os.path.join(
                "..",
                "processed",
                shape,
                year,
                shape + "_" + "sph_kgkg" + "_" + year + ".csv",
            )
        )
        ...
        vpmax_df.to_csv(
            os.path.join(
                ...
                shape,
                year,
                shape + "_vpdef_pa_" + year + ".csv",
            ),
            index=False,
        )
        total_max_negs = np.round(
            100
            * np.sum(vpmax_df.iloc[:, 2:].values < 0)
            / np.sum(vpmax_df.iloc[:, 2:].values != -9999.9999),
            3,
        )
        print("\nFor " + shape + " " + year + "...")
        print(
            str(total_max_negs)
            + " percent of (maximum) vapor pressure deficits are negative"
        )
