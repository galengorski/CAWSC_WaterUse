    ...
    def _to_df(in_data):
        ...
        while out_df is None:
            ...
            try:
                ...
                out_df = None
                logger.error(
                    "To DataFrame Export Error: "
                    "{}, FID:{}, ROW:{}, COL:{}, StartDate:{}, EndDate:"
                    "{}\n".format(
                        met_name, fid, row, col, start_date, end_date
                    ),
                    exc_info=False,
                )
                print("RETRYING {}".format(try_num))
                logger.info(
                    "Retrying export to df FID {}...Retry #{}".format(
                        fid, try_num
                    )
                )
            try_num += 1
    ...
    while out_df is None:
        ...
        try:
            ...
            logger.error(
                "To DataFrame Export Error: "
                "{}, FID:{}, ROW:{}, COL:{}, StartDate:{}, EndDate:"
                "{}\n".format(met_name, fid, row, col, start_date, end_date),
                exc_info=True,
            )
            print("RETRYING {}".format(try_num))
            logger.info(
                "Retrying export to df FID {}...Retry #{}".format(fid, try_num)
            )
        try_num += 1
