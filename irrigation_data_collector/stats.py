    ...
    for shape_ind, shape in enumerate(shapelist):
        for var in var_list:
            for year in year_list:
                ...
                month_arr = np.array(
                    [int(date.split("-")[1]) for date in load_file["Date"]]
                )
                for month in months:
                    ...
                    # [boolean, positional]
                    # needlessly complex
                    mean_df.iloc[int(month - 1), 2:] = np.nanmean(
                        load_file.loc[subset].iloc[:, 2:], axis=0
                    )
                    median_df.iloc[int(month - 1), 2:] = np.nanmedian(
                        load_file.loc[subset].iloc[:, 2:], axis=0
                    )
                    max_df.iloc[int(month - 1), 2:] = np.nanmax(
                        load_file.loc[subset].iloc[:, 2:], axis=0
                    )
                    min_df.iloc[int(month - 1), 2:] = np.nanmin(
                        load_file.loc[subset].iloc[:, 2:], axis=0
                    )
                mean_df.to_csv(
                    os.path.join(
                        out_path, shape + "_" + var + "_" + year + "_mean.csv"
                    ),
                    index=False,
                )
                median_df.to_csv(
                    os.path.join(
                        out_path,
                        shape + "_" + var + "_" + year + "_median.csv",
                    ),
                    index=False,
                )
                max_df.to_csv(
                    os.path.join(
                        out_path, shape + "_" + var + "_" + year + "_max.csv"
                    ),
                    index=False,
                )
                min_df.to_csv(
                    os.path.join(
                        out_path, shape + "_" + var + "_" + year + "_min.csv"
                    ),
                    index=False,
                )
