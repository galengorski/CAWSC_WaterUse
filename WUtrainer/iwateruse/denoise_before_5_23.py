...
def purify(df, **kwargs):
    ...
    for iter in range(N):
        ...
        new_score, new_df, df_move, signal_model = propose_sample_removal(df_signal, features, target,
                                                                          test_frac=test_frac, frac_noisy_samples=0.05,
                                                                          xgb_params=estimator.get_params(), score= score)
        frac_to_sampled = len(df_move)/len(df_signal)
        if frac_to_sampled < 1e-5:
            frac_to_sampled = 1e-5
        ...
        number_of_sigg = int(len(df_noise) * frac_to_sampled)
        sig_samples = df_noise.index.values[0:number_of_sigg]
