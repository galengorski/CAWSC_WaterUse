    ...
    with PdfPages(fn) as pdf:
        for feat in interp_feat:
            ...
            for h in huc2:
                ...
                PartialDependenceDisplay.from_estimator(features=interp_feat_,
                                                        estimator=estimator, X=XX[all_features], grid_resolution = 50,
                                                        **common_params
                                                        )
                ax = plt.gca()
...
def pdp_plots(estimator,df, fn, features ):
    ...
    with PdfPages(fn) as pdf:
        for feat in features:
            ...
            PartialDependenceDisplay.from_estimator(ax=ifig.gca(), features=interp_feat,
                                                    estimator=estimator, X=XX[features], grid_resolution = 50,
                                                    **common_params
                                                    )
...
def shap_plots(model, estimator, fn, features, data_frac = 0.5):
   ...
   with PdfPages(fn) as pdf:
        ...
        shap.summary_plot(shap_values, X100, max_display=15, show = False,  color_bar=False)
        ...
        cb.ax.text(0.5, -0.01, 'Low', transform=cb.ax.transAxes,
                   va='top', ha='center')
        cb.ax.text(0.5, 1.01, 'High', transform=cb.ax.transAxes,
                   va='bottom', ha='center')
        ...
        for feat in features:
            try:
                fig = plt.figure()
                cax = fig.gca()
                shap.plots.scatter( shap_values[:, feat], color=shap_values, ax= cax, hist=False,
                                    show=False)
                fig.suptitle("SHAP Values for {}".format(feat), fontsize=14)
                vrange = np.abs(X100[feat].quantile(0.90)/X100[feat].quantile(0.1))
                if vrange > 0:
        ...
        for h in hru2s:
            ...
            shap.summary_plot(shap_values, XX, max_display=15, color_bar=False, show=False)
            cb = plt.colorbar(shrink = 0.2, label="Feature Value")
