...
def compute_sampling_weight(
    err, error_quantile, outliers_sampling_method="equal_weight"
):
...
def propose_sample_removal(
    ...
    df = df_.copy()
    df = df.sample(
        frac=1, random_state=datetime.now().microsecond
    ).reset_index(drop=True)
    train_df = df.sample(
        frac=(1 - test_frac), random_state=datetime.now().microsecond
    )
    ...
    w, qq = compute_sampling_weight(
        err, error_quantile, outliers_sampling_method=outliers_sampling_method
    )
    ...
    return new_score, new_df, df_to_move, gb1, qq
...
def purify(df, **kwargs):
    ...
    estimator = _get_args(
        "estimator", default=xgb_estimator(params=None), **kwargs
    )
    ...
    df_signal = df.sample(
        frac=initial_split_frac, random_state=datetime.now().microsecond
    )
    ...
    for iter in range(N):
        ...
        try:
            df_noise["err"] = np.power(
                (signal_model.predict(df_noise[features]) - df_noise[target]),
                2,
            )
        except:
            xx = 1
        ...
        try:
            ...
            if nn > len(w[w > 0]):
                nn = len(w[w > 0])
            move_samples = choice(
                df_noise.index.values, nn, p=w, replace=False
            )
        except:
            xxxx = 1
