...
def propose_sample_removal(df_, features, target, test_frac=0.3, outliers_sampling_method='equal_weight',
                           ...
                           score='neg_mean_absolute_percentage_error', xgb_params=None):
    """

    """
    # shuffle and split
    df = df_.copy()
    df = df.sample(frac=1, random_state = datetime.now().microsecond).reset_index(drop=True)
    train_df = df.sample(frac=(1 - test_frac), random_state = datetime.now().microsecond)
...
def purify(df, **kwargs):
    ...
    df = df.sample(frac=1, random_state = datetime.now().microsecond).reset_index(drop=True)
    df_signal = df.sample(frac=initial_split_frac, random_state = datetime.now().microsecond)
    ...
    for iter in range(N):
        ...
        try:
            new_score, new_df, df_move, signal_model, error_upper_threshold = propose_sample_removal(df_signal, features,
                                                                                                     target, error_quantile = 0.90,
                                                                                                     test_frac=test_frac,
