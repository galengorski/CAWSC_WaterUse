...
def generate_metrics(y_true, y_pred):
    ...
    scores.append(
        np.power(mean_squared_error(y_true=y_true, y_pred=y_pred), 0.5)
    )
    names.append("rmse")
...
def generat_metric_by_category(
    estimator, X, y_true, features, category="HUC2"
):
...
def complete_model_diagnose(
    model, estimator=None, basename="initial", monthly=False
):
    """
    ...
    if monthly:
        ...
        figfile = os.path.join(
            fig_folder, "1_annual_1to1_{}.pdf".format(basename)
        )
    else:
    ...
    df_metric.to_csv(
        os.path.join(ws, "2_performance_metric_{}.csv".format(basename)),
        index=False,
    )
    ...
    # -----------------------
    perf_df_huc2 = generat_metric_by_category(
        estimator, X_test, y_test, features=features, category="HUC2"
    )
    perf_df_huc2.to_csv(
        os.path.join(ws, "3_performance_by_huc2_{}.csv".format(basename)),
        index=False,
    )
    perf_df_state = generat_metric_by_category(
        estimator, X_test, y_test, features=features, category="state_id"
    )
    perf_df_state.to_csv(
        os.path.join(ws, "4_performance_by_state_{}.csv".format(basename)),
        index=False,
    )
    ...
    for err_type in ["rmse", "r2", "mape"]:
        figfile = os.path.join(
            fig_folder,
            "5_validation_{}_by_huc2_{}.pdf".format(err_type, basename),
        )
        df_shp = figures.plot_huc2_map(
            shp_file=huc2_shp_file,
            usa_map=usa_conus_file,
            info_df=perf_df_huc2,
    ...
    figfile = os.path.join(
        fig_folder, "6_validation_by_huc2_{}.pdf".format(basename)
    )
    ...
    if monthly:
        ...
        fig_info[
            "super_title"
        ] = "Validation Error for Estimated Monthly Fraction"
        fig_info["vmax"] = 0.18
        fig_info["vmin"] = 0.03

        figfile = os.path.join(
            fig_folder, "7_map_val_error_{}.pdf".format(basename)
        )
        figures.plot_multiple_scatter_map(
            df_,
            xcol="LONG",
            ycol="LAT",
        ...
        df_shp = figures.plot_scatter_map(
            X_test["LONG"],
            X_test["LAT"],
            err,
            legend_column=legend_column,
        ...
        figfile = os.path.join(
            fig_folder, "6_map_val_error_{}.shp".format(basename)
        )
    ...
    for type in importance_types:
        figfile = os.path.join(
            fig_folder, "8_feature_importance_{}_{}.pdf".format(type, basename)
        )
        ...
        figures.feature_importance(
            estm, max_num_feature=15, type=type, figfile=figfile
        )
...
def compute_temporal_change(model):
    ...
    for state in states:
        ...
        curr_state["del_pcgd"] = (
            curr_state["est_per_capita"].diff() / curr_state["est_per_capita"]
        ) * curr_state["est_per_capita"]
    ...
    # counties
    all_counties = model.df_pred[
        ["county_id", "Year", "est_per_capita", "pop"]
    ]
    all_counties["total_wu"] = (
        all_counties["pop"] * all_counties["est_per_capita"]
    )
    all_counties = all_counties.groupby(by=["county_id", "Year"]).sum()
    all_counties.reset_index(inplace=True)
    all_counties["est_per_capita"] = (
        all_counties["total_wu"] / all_counties["pop"]
    )
...
def complete_monthly_model_eval(model, estimator=None, basename="initial"):
    ...
    figfile = os.path.join(
        fig_folder, "1_model_monthly_fractions_{}.pdf".format(basename)
    )
    df_shp = figures.plot_multiple_scatter_map(
        df_pred,
        xcol="LONG",
        ycol="LAT",
        legend_column=legend_column,
    ...
    figfile = os.path.join(
        fig_folder, "2_boxplot_by_huc2_{}.pdf".format(basename)
    )
    figures.plot_box_plot_for_monthly_fractions(df_pred, figfile=figfile)

    # time series of HUC2 per capita (or fraction)
    figfile = os.path.join(
        fig_folder, "3_average_MF_timeseries_by_huc2_{}.pdf".format(basename)
    )
...
def complete_model_evaluate(model, estimator=None, basename="initial"):
    ...
    if 0:
        ...
        awuds_df["COUNTYCODE"] = (
            awuds_df["COUNTYCODE"].astype(str).str.zfill(3)
        )
        awuds_df["county_id"] = (
        ...
        awuds_county_pc = (
            awuds_df_[["county_id", "awuds_pc", "PS-WTotl", "PS-TOPop"]]
            .groupby(by="county_id")
            .mean()
        )

        model_county_pc = (
            df_pred[["county_id", "pop", "wu_rate"]]
            .groupby(by=["county_id"])
            .sum()
        )
        model_county_pc.reset_index(inplace=True)
        model_county_pc["county_id"] = model_county_pc["county_id"].astype(str)
        model_county_pc["county_id"] = model_county_pc["county_id"].str.zfill(
            5
        )
        model_county_pc["pc"] = (
            model_county_pc["wu_rate"] / model_county_pc["pop"]
        )
    ...
    figfile = os.path.join(
        fig_folder, "1_total_wu_change_with_time_{}.pdf".format(basename)
    )
    ...
    figfile = os.path.join(
        fig_folder,
        "2_national_per_capita_change_with_time_{}.pdf".format(basename),
    )
    figures.plot_national_pc_change(
        us_annual, x="Year", y="pc", figfile=figfile
    )
    ...
    figfile = os.path.join(
        fig_folder,
        "3_per_capita_change_with_time_for_each_huc2_{}.pdf".format(basename),
    )
    figures.plot_timeseries_per_huc2(
        df=all_huc2s, x="Year", y="est_per_capita", figfile=figfile
    )
    ...
    # -----------------------
    figfile = os.path.join(
        fig_folder, "4_per_capita_hist_each_huc2_{}.pdf".format(basename)
    )
    ...
    # -----------------------
    av_per_capita = df_pred.groupby(by="sys_id").mean()[
        ["LONG", "LAT", "est_per_capita"]
    ]
    figfile = os.path.join(
        fig_folder, "5_mean_service_area_per_capita_{}.pdf".format(basename)
    )
    df_shp = figures.plot_scatter_map(
        av_per_capita["LONG"],
        av_per_capita["LAT"],
        av_per_capita["est_per_capita"],
        legend_column="est_per_capita",
        cmap="jet",
        title="Per Capita WU",
        figfile=figfile,
        log_scale=False,
    ...
    figfile = os.path.join(
        fig_folder, "5_mean_service_area_per_capita_{}.shp".format(basename)
    )
    df_shp.to_file(figfile)
    ...
    usa_conus_file = files_info["usa_conus_file"]
    summary_df = (
        model.df_pred[["HUC2", "est_per_capita"]].groupby(by="HUC2").describe()
    )
    ...
    df_temp = (
        model.df_pred[["HUC2", "pop", "tot_wu"]].groupby(by=["HUC2"]).sum()
    )
    ...
    figfile = os.path.join(
        fig_folder, "6_mean_huc2_per_capita_{}.pdf".format(basename)
    )
    df_shp = figures.plot_huc2_map(
        shp_file=huc2_shp_file,
        usa_map=usa_conus_file,
        ...
        epsg=5070,
        cmap="Greens",
        title="Average Per Capita by HUC2",
        figfile=figfile,
    )
    figfile = os.path.join(
        fig_folder, "6_huc2_stats_per_capita_{}.pdf".format(basename)
    )
    df_shp.to_file(figfile + ".shp")
    ...
    summary_df = (
        model.df_pred[["state_id", "est_per_capita"]]
        .groupby(by="state_id")
        .describe()
    )
    cols = summary_df.columns
    ...
    df_temp = (
        all_states[["state_id", "pop", "total_wu"]]
        .groupby(by=["state_id"])
        .mean()
    )
    ...
    figfile = os.path.join(
        fig_folder, "7_mean_state_per_capita_{}.pdf".format(basename)
    )
    df_shp = figures.plot_state_map(
        shp_file=state_shp_file,
        usa_map=usa_conus_file,
        info_df=summary_df,
        legend_column="wmean",
        log_scale=False,
        epsg=5070,
        cmap="Purples",
        title="Average Per Capita by State",
        figfile=figfile,
        polygon_col_id="GEOID",
    )
    figfile = os.path.join(
        fig_folder, "7__state_stats_per_capita_{}.shp".format(basename)
    )
    df_shp.to_file(figfile)

    figfile = os.path.join(
        fig_folder, "8_mean_state_total_wu_{}.pdf".format(basename)
    )
    df_shp = figures.plot_state_map(
        shp_file=state_shp_file,
        usa_map=usa_conus_file,
        ...
        epsg=5070,
        cmap="winter",
        title="Average Total Water Use by State",
        figfile=figfile,
        polygon_col_id="GEOID",
    )
    ...
    usa_conus_file = files_info["usa_conus_file"]
    figfile = os.path.join(
        fig_folder, "9_mean_county_per_capita_{}.pdf".format(basename)
    )
    summary_df = (
        model.df_pred[["county_id", "est_per_capita"]]
        .groupby(by="county_id")
        .describe()
    )
    cols = summary_df.columns
    ...
    summary_df.columns = renamed_cols
    df_temp = (
        model.df_pred[["county_id", "pop", "tot_wu"]]
        .groupby(by=["county_id"])
        .sum()
    )
    df_temp["wpc"] = df_temp["tot_wu"] / df_temp["pop"]
    summary_df["wmean"] = df_temp["wpc"]
    summary_df.reset_index(inplace=True)
    df_shp = figures.plot_county_map(
        shp_file=county_shp_file,
        usa_map=usa_conus_file,
        info_df=summary_df,
        legend_column="wmean",
        log_scale=False,
    ...
    figfile = os.path.join(
        fig_folder, "9__county_stats_per_capita_{}.shp".format(basename)
    )
    df_shp.to_file(figfile)
    ...
    usa_conus_file = files_info["usa_conus_file"]
    figfile = os.path.join(
        fig_folder, "10_pc_average_change_by_state_{}.pdf".format(basename)
    )
    ...
    figfile = os.path.join(
        fig_folder, "10_pc_average_change_by_state_{}.shp".format(basename)
    )
    df_shp.to_file(figfile)
