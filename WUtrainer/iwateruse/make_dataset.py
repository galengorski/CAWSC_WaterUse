...
def get_annual_wu(model, annual_wu):
    ...
    model.log.to_table(
        cleanned_wu[cols].describe(percentiles=[0.05, 0.5, 0.95]), header=-1
    )
...
def make_ds_per_capita_basic(model, datafile=None):
    ...
    annual_wu = pd.read_csv(
        r"C:\work\water_use\CAWSC_WaterUse\etc\annual_wu.csv"
    )
...
def make_ds_per_capita_system_average(model):
    # ======= Load =======
    features_info = pd.read_excel(
        r"C:\work\water_use\ml_experiments\annual_v_0_0\features_status.xlsx",
        sheet_name="annual",
    )
    model.df_train_bk = pd.read_csv(
    ...
    # custom fixes
    model.df_train_bk["rur_urb_cnty"] = model.df_train_bk[
        "rur_urb_cnty"
    ].astype(int)

    # Extract data
    model.categorical_features = features_info[
        features_info["Type"].isin(["categorical"])
    ...
    numeric_feat = set(model.df_train.columns).difference(
...
def make_ds_per_capita_system_average_neutral(model):
    ...
    # custom fixes
    model.df_train_bk = model.df_train_bk[
        model.df_train_bk["Ecode"].isin(["N"])
    ]
    model.df_train_bk["rur_urb_cnty"] = model.df_train_bk[
        "rur_urb_cnty"
    ].astype(int)
