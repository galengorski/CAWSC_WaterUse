...
def propose_sample_removal(
    ...
    relative_error = (
        np.abs(gb1.predict(test_df[features]) - test_df[target])
    ) / np.abs(test_df[target])
...
def purify(df, **kwargs):
    ...
    min_mse = _get_args(
        "min_mse", default=xgb_estimator(params=None), **kwargs
    )
    ...
    for iter in range(N):
        ...
        df_noise["err"] = np.power(
            (signal_model.predict(df_noise[features]) - df_noise[target]), 2
        )
