...
def summarize_missing_data(df=None, target="", plot=False):
    ...
    missing_df = missing_df.sort_values(
        by=["n"], ascending=False, inplace=False
    )
...
def fill_missing_annual_climate_data(
    df, sys_id_field, climate_variables, x_field, y_field
):
    """
...
def fill_missing_data_using_xgboost(
    df_in, impute_col, target=None, is_categorical=False
):
...
if __name__ == "__main__":
    ...
    feature_info = pd.read_excel(
        os.path.join(workspace, config.get("Features", "features_info_file"))
    )

    train_df = annual_db[annual_db[target_col] > 0]

    features_to_drop = feature_info.loc[
        feature_info["Not_Feature"] == 1, "Feature_name"
    ].values.tolist()
    features_to_drop = (
        features_to_drop
        + feature_info.loc[
            feature_info["Skip"] == 1, "Feature_name"
        ].values.tolist()
    )
