...
def generate_wu_per_capita_spatial_stat(df, pop_field , max_points = 50, raduis=500):
    ...
    for sys_id in tqdm(sys_ids):
        ...
        pc_stat_df.append([sys_id, x0, y0, ag_pc, pc_mean, pc_std, pc_median, pc_trim_mean, p95, p5])
    pc_stat_df = pd.DataFrame(pc_stat_df, columns=['sys_id', 'x', 'y', 'ag_pc', 'pc_mean', 'pc_std', 'pc_median', 'pc_trim_mean', 'p95', 'p5' ])
    pc_stat_df.to_csv(r"C:\work\water_use\mldataset\ml\training\misc_features\spatial_features\pc_50_withTNin.csv")
...
if __name__ == "__main__":
    ...
    raw_train_db = pd.read_csv(r"C:\work\water_use\mldataset\ml\training\train_datasets\Annual\raw_wu_annual_training.csv")

    if 0:
        raw_train_db = fix_house_negative_age(raw_train_db)
        raw_train_db = fix_negative_income(raw_train_db)
        raw_train_db.to_csv(r"C:\work\water_use\mldataset\ml\training\train_datasets\Annual\raw_wu_annual_training.csv")
    # =======================================
    ...
    generate_wu_per_capita_spatial_stat(train_db, pop_field = 'swud_pop', max_points=50, raduis=500)
