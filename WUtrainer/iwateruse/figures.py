...
def one_to_one(y_actual, y_hat, heading, xlabel, ylabel, figfile):
    with styles.USGSPlot():
        ...
        plt.plot(
            [min(y_actual), max(y_actual)], [min(y_actual), max(y_actual)], "r"
        )
        accuracy = r2_score(y_actual, y_hat)
        rmse = np.power(mean_squared_error(y_actual, y_hat), 0.5)
        styles.heading(ax=ax, heading=heading, idx=0, fontsize=16)
        msg = r"$r^{2}$" + " = {:.2f}".format(np.round(accuracy, 2))
        msg = msg + "\nrmse = {} GPCD".format(np.round(rmse, 2))
        styles.add_text(
            ax=ax, text=msg, x=0.02, y=0.92, backgroundcolor="white"
        )
...
def feature_importance(estimator, max_num_feature, type, figfile):
    ...
    with styles.USGSPlot():
        ...
        f = plot_importance(
            ax=ax,
            booster=estimator,
            max_num_features=max_num_feature,
            importance_type=type,
        )
        styles.heading(ax=ax, heading="Feature Importance", idx=0, fontsize=16)
...
def plot_pc_hist_per_huc2(df, figfile):
    ...
    fig.text(
        0.08, 0.5, "density", va="center", rotation="vertical", fontsize=12
    )
...
def time_bars(df_, x, y, figfile=""):
    ...
    with styles.USGSMap():
        df[y] = df[y] / 1e6
        bars = ax.bar(x=df[x].values, height=df[y].values)
        ...
        ax.set_ylabel(
            "Total Water Use - Million of Gallons Per Day (mgpd)",
            labelpad=15,
            color="#333333",
        )
    ...
    with styles.USGSMap():
        if log_scale:
            norm = matplotlib.colors.LogNorm(
                vmin=shp_df[legend_column].min(),
                vmax=shp_df[legend_column].max(),
            )
            ax = shp_df.plot(
                ax=ax,
...
def plot_county_map(
    ...
    usa_bkg.to_crs(epsg=epsg, inplace=True)
    info_df["county_id"] = (
        info_df["county_id"].astype(int).astype(str).str.zfill(5)
    )
    shp_df = shp_df[[polygon_col_id, "geometry"]]
    shp_df = shp_df.merge(
        info_df, how="left", left_on=polygon_col_id, right_on="county_id"
    )

    if log_scale:
        norm = matplotlib.colors.LogNorm(
            vmin=shp_df[legend_column].min(), vmax=shp_df[legend_column].max()
        )
...
def plot_state_map(
    ...
    usa_bkg.to_crs(epsg=epsg, inplace=True)
    info_df["state_id"] = (
        info_df["state_id"].astype(int).astype(str).str.zfill(2)
    )
    shp_df = shp_df[[polygon_col_id, "geometry"]]
    shp_df = shp_df.merge(
        info_df, how="left", left_on=polygon_col_id, right_on="state_id"
    )

    if log_scale:
        norm = matplotlib.colors.LogNorm(
            vmin=shp_df[legend_column].min() - 1,
            vmax=shp_df[legend_column].max() + 1,
        )
...
def plot_scatter_map(
    ...
    if log_scale:
        norm = matplotlib.colors.LogNorm(
            vmin=df_shp[legend_column].min(), vmax=df_shp[legend_column].max()
        )
...
def plot_multiple_scatter_map(
    df, xcol, ycol, legend_column, fig_info, figfile
):
    ...
    lat = df_[ycol]
    df_shp = geopandas.GeoDataFrame(
        df_, geometry=geopandas.points_from_xy(lon, lat, crs="EPSG:4326")
    )  # 2163
    ...
    with styles.USGSMap():
        for month in range(1, 13):
            ...
            styles.heading(
                ax=ax, letter=months[n], heading="", idx=0, fontsize=10
            )
...
def plot_timeseries_monthly_frac(df, figfile):
    ...
    super_title = (
        "Temporal Change of HUC2 Averaged \n Water Use Monthly Fractions"
    )
    df_ = df.groupby(by=["sys_id", "Month"]).mean()
    df_.reset_index(inplace=True)
    nrows = 3
...
def residual_vs_fitted(model, estimator, figfile):
    ...
    plot_lm_2 = QQ.qqplot(
        line="45", alpha=0.5, color="#4C72B0", lw=1, ax=axes[1]
    )
