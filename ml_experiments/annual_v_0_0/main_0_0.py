...
def initialize(model, config_file):
    ...
    append_date_to_log_file = model.config.get(
        "Log", "append_date_to_file_name"
    )
...
def load(model):
    ...
    model.log.info(
        "loading: {}".format(
            os.path.join(
                model.workspace,
                model.config.get("Features", "features_info_file"),
            )
        )
    )
    feature_info_df = pd.read_excel(
        os.path.join(
            model.workspace, model.config.get("Features", "features_info_file")
        ),
        sheet_name="annual",
    )
    model.feature_info_df = feature_info_df
    model.features = model.feature_info_df[feature_info_df["Skip"] != 1][
        "Feature_name"
    ].values.tolist()
    ...
    if model.type in ["monthly"]:
        df_main.loc[
            df_main["monthly_fraction"] <= 0, "monthly_fraction"
        ] = np.NAN
    else:
...
def clean(model):
    ...
    features_not_in_data_base = set(
        model.feature_info_df["Feature_name"]
    ).difference(set(model.df_train.columns))
    dff = list_to_df(list(features_not_in_data_base), 5)
    model.log.to_table(
        dff, header=len(dff), title="Columns not in the database ..."
    )

    features_not_in_feature_list = set(model.df_train.columns).difference(
        set(model.feature_info_df["Feature_name"])
    )
    dff = list_to_df(list(features_not_in_feature_list), 5)
    model.log.to_table(
        dff, header=len(dff), title="Columns not in the features list ..."
    )

    if model.type in ["annual"]:
        feature_mask = ~(
            (model.feature_info_df["Skip"] == 1)
        )  # | (model.feature_info_df['Not_Feature'] == 1))
    elif model.type in ["monthly"]:
        feature_mask = ~((model.feature_info_df["monthly Skip"] == 1))
    else:
        raise ValueError("Unknown model type...")

    not_features = model.feature_info_df[
        ~(model.feature_info_df["Not_Feature"] == 1)
    ]["Feature_name"].values.tolist()
    features_to_drop = model.feature_info_df[~feature_mask][
        "Feature_name"
    ].values.tolist()
    features_to_drop = (
        set(features_to_drop).union(features_not_in_feature_list)
    ).difference(features_not_in_data_base)
    ...
    model.log.to_table(
        model.df_train,
        title="Training data -- shape = {}".format(model.df_train.shape),
    )

    """ # Summarize missing data"""
    model.log.info("Missing data")
    missing_df = data_cleaning.summarize_missing_data(
        df=model.df_train, target=model.target
    )
    model.log.to_table(
        missing_df, title="Missing data ", header=len(missing_df)
    )
    ...
    if model.impute_method in ["none"]:
        ...
        cat_features = model.feature_info_df[
            model.feature_info_df["Type"] == "categorical"
        ]["Feature_name"].values
        for impute_col in missing_df["index"].values:
            ...
            imputed_feature = data_cleaning.fill_missing_data_using_xgboost(
                df_in, impute_col, is_categorical=flg
            )
    ...
    model.log.info(
        "Clean database is generated at {} ....".format(fn_clean_df)
    )
...
def main(config_file):
    ...
    # preprocess(model)
    model.clean_file_name = os.path.abspath(
        model.config.get("Files", "cleaned_training_file")
    )
    ...
    if clean_the_data:
        ...
        model.log.info(
            "Clean database is read from {} ....".format(model.clean_file_name)
        )
