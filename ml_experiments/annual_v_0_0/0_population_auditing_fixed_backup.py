...
if generate_pop_info:
    ...
    pop_info.to_csv(r"C:\work\water_use\mldataset\ml\training\train_datasets\Annual\pop_and_others2.csv", 
                   index=False)
...
f = axs[0].scatter(np.log10(pop_info['pop_swud15']), 
...
dn50 = v.quantile(0.5)
display(Markdown('The median of the density {}, 95% is {}, and 5% is {}'
                 .format(int(dn50),int(dn95), int(dn05) )))
...
# Plot
    # Plot histogram
v.plot(kind = "hist", density = True, alpha = 0.65, bins = 200) # change density to true, because KDE uses density
    # Plot KDE
v.plot(kind = "kde")
...
# %%
pc_xy_plc = pd.read_csv(r"C:\work\water_use\mldataset\ml\training\misc_features\spatial_features\pc_50_plc_pop_interpolated.csv")
pc_xy_swud = pd.read_csv(r"C:\work\water_use\mldataset\ml\training\misc_features\spatial_features\pc_50_pop_swud15.csv")

# %%
plt.figure()
plt.scatter(pc_xy_plc['x'], pc_xy_plc['y'], c=pc_xy_plc['pc_median'], s=1, cmap='jet')
...
# %%
plt.figure()
plt.scatter(pc_xy_swud['x'], pc_xy_swud['y'], c=pc_xy_swud['pc_median'], s=1, cmap='jet')
...
pop_info = pop_info.merge(sys_id_Cordinates, right_on = ['sys_id', 'Year'], left_on = ['sys_id', 'year'], how = 'left')
del(pop_info['year'])
