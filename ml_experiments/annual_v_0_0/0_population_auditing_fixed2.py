...
pop_wsa_bg_info = pop_wsa_bg_info.merge(
    df_main[["sys_id", "Year", "population"]],
    how="left",
...
very_small_sys = master_pop_df[master_pop_df["pop"].isna()][
    "WSA_AGIDF"
].unique()
mean_swud = (
    master_pop_df[["WSA_AGIDF", "TPOPSRV", "population_c"]]
    .groupby("WSA_AGIDF")
    .mean()
)
...
for wsys in w_sys_no_bdg:
    ...
    tpopsrv = master_pop_df[sys_mask][
        "TPOPSRV"
    ].mean()  # 15 and 16 population are the same

    if not (np.isnan(popu_c)):
        if not (np.isnan(tpopsrv)):
            pop_new = (
                tpopsrv / popu_c * master_pop_df[sys_mask]["population_c"]
            )
