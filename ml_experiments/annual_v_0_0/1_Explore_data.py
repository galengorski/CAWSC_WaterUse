...
plt.figure()
sns.distplot(
    np.log10(df_train[target] / df_train[pop_field]),
...
plt.figure()
plt.scatter(
    np.log10(db_[pop_field]),
    np.log10(db_["wu_rate"]),
    c=np.log10(db_[pop_field] / db_["WSA_SQKM"]),
    s=5,
)
plt.colorbar()
plt.show()
...
f, ax = plt.subplots(figsize=(8, 6))
sns.distplot(
    pc25, label="Cold, no dry season, hot summer", rug=True, hist=False
)
...
sns.distplot(
    pc8, label="Temperate, dry summer, hot summer ", rug=True, hist=False
)
sns.distplot(
    pc14, label="Temperate, no dry season, hot summer", rug=True, hist=False
)
plt.xlim([-1, 3.5])
plt.legend()
