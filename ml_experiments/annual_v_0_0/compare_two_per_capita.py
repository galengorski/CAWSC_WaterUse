...
Qdataset = pd.read_csv(r"C:\work\water_use\ml_experiments\annual_v_0_0\clean_train_db.csv")
Qpop_info = pd.read_csv(r"pop_info.csv")
Qpc_50_swud = pd.read_csv(r"C:\work\water_use\mldataset\ml\training\misc_features\spatial_features\pc_50_pop_swud15.csv")
Qpc_50_plc = pd.read_csv(r"C:\work\water_use\mldataset\ml\training\misc_features\spatial_features\pc_50_plc_pop_interpolated.csv")

Qdf_ = Qpc_50_swud[['sys_id', 'pc_median' ]]
Qdataset = dataset.merge(Qdf_, right_on=['sys_id'], left_on=['sys_id'] , how = 'left')
del(Qdf_)
...
Qdataset = Qdataset.merge(Qpop_df, right_on=['sys_id', 'Year'], left_on=['sys_id', 'Year'] , how = 'left')
...
QX_train, QX_test, Qy_train, Qy_test = train_test_split(QX, Qy, test_size=0.2, random_state = 123)
