...
# =============================
if 1:
    selected_features = model.load_features_selected(
        method="xgb_total_cover",
...
model.log.to_table(
    final_dataset[features].describe(percentiles=[0, 0.05, 0.5, 0.95, 1]),
    "Features",
    header=-1,
)
model.log.to_table(
    final_dataset[[target]].describe(percentiles=[0, 0.05, 0.5, 0.95, 1]),
    "Target",
    header=-1,
)

X_train, X_test, y_train, y_test = splittors.stratified_split(
    model, test_size=0.3, id_column="HUC2", seed=123
)
...
if train_initial_model:
    ...
    if plot_diagnosis:
        model_diagnose.complete_model_diagnose(
            model, estimator=trained_gb, basename="initial", monthly=True
        )
...
if use_boruta_results:
    ...
    selection_eval_results.to_csv(
        r"results\feature_selection_summary_monthly.csv"
    )
...
if train_denoised_model:
    ...
    model.log.info(
        "**** Outliers file name : {}".format(files_info["outliers_file_used"])
    )
    ...
    df_metric.to_csv(
        os.path.join(model.model_ws, "metric_denoised_all_features.csv"),
        index=False,
    )
    model.log.to_table(
        df_metric, title="Metrics of denoised model with selected features"
    )

    # diagnose model
    model_diagnose.complete_model_diagnose(
        model, estimator=gb, basename="denoised", monthly=True
    )

    # save model
    model_file_name = os.path.join(
        model.model_ws, "denoised_model_with_selected_features.json"
    )
