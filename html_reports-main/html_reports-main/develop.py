...
tmax = np.random.random(365) * 100.0
tmin = tmax - 25.0
mgal = np.random.random(365) * 100
sdate = dt.datetime(2020, 1, 1)
date = [sdate + dt.timedelta(days=i) for i in range(365)]

d = {"tmax": tmax, "tmin": tmin, "mgal": mgal, "date": date}

df = pd.DataFrame.from_dict(d)

section = ReportSection(
    "Machine learning model report development",
    "Input data",
...
section2.geojson_to_heatmap(
    hucs,
    "properties.feat_name",
    df2.huc12,
    df2.population,
    colorscale="Viridis",
    zmin=0,
...
html = "".join(htmls)
with open(os.path.join("examples", "development_example.html"), "w") as foo:
    foo.write(html)
