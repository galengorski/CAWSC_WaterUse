...
annual_training_file = os.path.join(db_root, r"ml\training\train_datasets\annual\wu_annual_training2.csv")
monthly_training_file = os.path.join(db_root, r"C:\work\water_use\mldataset\ml\training\train_datasets\Monthly\wu_monthly_training2.csv")
swud_df = pd.read_csv(os.path.join(db_root, r"ml\training\targets\monthly_annually\swud_v15.csv"),  encoding='cp1252')
swud_df16 = pd.read_csv(os.path.join(db_root, r"ml\training\targets\monthly_annually\SWUDS v16 WDs by FROM-sites and by WSA_AGG_ID_04282021.csv"),  encoding='cp1252')
...
koppen_climate_df = geopandas.read_file(r"C:\work\water_use\mldataset\gis\koppen_maps\wsa_climate_intersection_proj.shp")
...
wsa_local_pop = pd.read_excel(r"C:\work\water_use\mldataset\ml\training\misc_features\V1_polys_with_water_service_08072021_for_Ayman_popsrv_year.xlsx")
...
if add_zillow_data:
    ...
    for file in zillow_files:
        ...
        cols = ['WSA_AGIDF', 'zill_nhouse',  'LotSizeSquareFeet_sum', 'YearBuilt_mean', 'BuildingAreaSqFt_sum', 'TaxAmount_mean', 'NoOfStories_mean' ]
        zdf = zdf[cols]
...
if  add_local_pop:
    ...
    train_db = train_db.merge(wsa_local_pop, how='left', left_on='sys_id', right_on='WSA_AGIDF')
    del (train_db['WSA_AGIDF'])
...
if add_places_pop:
    ...
    global counter11
    counter11 = 0
    cols = ['POPESTIMATE2010', 'POPESTIMATE2011',
    ...
    def compute_place_census(_df):
       ...
       'POPESTIMATE2018', 'POPESTIMATE2019', 'POPESTIMATE2020']
        ...
        if len(d)>0:  # use transeint data
            ...
            df_out.loc[mask3, 'pop'] = df_out[mask3]['ratio'].values * df_out.loc[yr]['pop']
            ...
            if _df['plc_population'].values[0]>0:
                ...
                df_out['pop'] = _df['plc_population'].values[0] * df_out['ratio']
    ...
    train_db = train_db.merge(places_pop_df, left_on='sys_id', right_on='WSA_AGIDF', how='left')
    del(train_db['WSA_AGIDF'])
    ...
    if Parallel:
        ...
        xx = sd.groupby(['sys_id'], group_keys=False).apply(compute_place_census)
        yy = xx.compute(scheduler='processes')
        endd = time.time()
    else:
        start = time.time()
        yy = train_db.groupby(['sys_id'], group_keys=False).apply(compute_place_census)
        endd = time.time()
    print(" Running Time is {}".format(endd - start))
    yy.reset_index(inplace=True)

    train_db = train_db.merge(yy,left_on=['sys_id', 'Year'], right_on=['sys_id', 'year'], how='left')
    for col in cols:
        del(train_db[col])
    train_db.to_csv(annual_training_file, index=False)
...
if add_census_place_data:
    if 0:
        census_places_folder = r"C:\work\water_use\data\WSA_places\WSA_places\WSA_places"
        ...
        for c_ in all_dfs.columns:
            ...
            del (all_dfs[c_])
        train_db = train_db.merge(all_dfs, left_on=['sys_id', 'Year'], right_on=['plc_sys_id', 'plc_year'], how='left')
    else:
        ...
        train_db = train_db.merge(train_db_o[plc_columns], how='left', right_on=['plc_year', 'plc_sys_id'],
                                  left_on=['Year', 'sys_id'])
    ...
    pop_info = train_db.groupby(['sys_id'], group_keys=False).apply(compute_place_census)
    endd = time.time()
    vv=1
xx=1