...
class ServiceArea(object):
    ...
    def get_sev_areas_with_wu_data(self):
        ...
        for irow, row in self.wu_df.iterrows():
            ...
            if (pws_id in self.shp_df["WSA_AGG_ID"].values) and not (
                pd.isna(pws_id)
            ):
                ...
                self.wu_df.loc[
                    self.wu_df["T_PWSID"].values == pws_id, "GNIS_ID"
                ] = int(gnis)
                found_list.append(pws_id)
                continue

            if (sid in self.shp_df["USGS_SID"].values) and not (pd.isna(sid)):
                gnis = self.shp_df.loc[
                    self.shp_df["USGS_SID"].values == sid, "GNIS_ID"
                ].values[0]
                if np.isnan(gnis):
                    fidw.write("{}---{}".format(pws_id, sid))
                    fidw.write("\n")
                ...
                self.wu_df.loc[
                    self.wu_df["T_SID"].values == sid, "GNIS_ID"
                ] = int(gnis)
        ...
        fidw.close()
        self.wu_df.to_csv(
            r"D:\Workspace\projects\machine_learning\data\dataset\WU_Data\water_use_data_with_shp.csv"
        )
...
if __name__ == "__main__":
    ...
    sa = ServiceArea(
        serv_data_excel=srv_info_file, shp_attribute=shp_attribute
    )
