...
def add_county_data(trn_df=None):
    annual_db_root = r"C:\work\water_use\mldataset\ml"
    PARAMETERS = utl.load_parameters(
        os.path.join(annual_db_root, r"training\misc_features\parameters.txt")
    )

    # load existing training data
    if trn_df is None:
        trn_data = os.path.join(
            annual_db_root,
            r"training\train_datasets\Annual\wu_annual_training3.csv",
        )
    ...
    trn_df = utl.join_data(
        trn_df, wsa_df, left_on="sys_id", right_on="wsa_agidf"
    )
