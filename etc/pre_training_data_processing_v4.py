...
model = Model(
    name="annual_pc",
    log_file="assemble_annual_data.log",
    feature_status_file=feature_status_file,
)
model_monthly = Model(
    name="monthly_frac",
    log_file="assemble_monthly_data.log",
    feature_status_file=feature_status_file,
    model_type="monthly",
)
...
annual_training_file = os.path.join(
    db_root, r"ml\training\train_datasets\annual\wu_annual_training3.csv"
)
...
swud_df = pd.read_csv(
    os.path.join(
        db_root, r"ml\training\targets\monthly_annually\SWUDS_v17.csv"
    ),
    encoding="cp1252",
)
nonswuds_df = pd.read_csv(
    os.path.join(
        db_root,
...
joint_db = pd.read_csv(
    r"C:\work\water_use\mldataset\ml\training\targets\monthly_annually\Join_swud_nswud3.csv"
)
...
sell_buy_df = pd.read_csv(
    r"C:\work\water_use\mldataset\ml\training\misc_features\selling_buying\water_exchange_data.csv",
    encoding="cp1252",
)
...
ag_pop_year_df = pd.read_csv(
    os.path.join(
        db_root,
        r"ml\training\misc_features\V1_polys_with_water_service_06022021_for_GIS.csv",
    ),
    encoding="cp1252",
)  # ?#
...
monthly_climate = pd.read_csv(
    r"C:\work\water_use\mldataset\ml\training\features\monthly_climate.csv"
)
annual_climate = pd.read_csv(
    r"C:\work\water_use\mldataset\ml\training\features\annual_climate.csv"
)
county_shp = geopandas.read_file(
    r"C:\work\water_use\mldataset\gis\tl_2016_us_county\tl_2016_us_county.shp"
)
annual_wu = pd.read_csv(r"annual_wu.csv")
monthly_wu = pd.read_csv(r"monthly_wu.csv")
cii_fractions_df = pd.read_csv(
    r"C:\work\water_use\mldataset\ml\training\misc_features\cii\national_cii_dpc_ml_results.csv"
)
thermo_2010 = pd.read_excel(
    r"C:\work\water_use\mldataset\ml\training\misc_features\thermo\TEP_2010_WaterSouceMunicipal.xlsx"
)
thermo_2015 = pd.read_excel(
    r"C:\work\water_use\mldataset\ml\training\misc_features\thermo\TEP_2015_WaterSourceMunicipal.xlsx"
)
...
if collect_all_monthly_data:
    ...
    train_db_monthly = train_db_monthly.drop_duplicates(
        subset=["Year", "sys_id"], keep="last"
    ).reset_index(drop=True)
    for mo in range(1, 13):
        train_db_monthly[mo] = mo
    train_db_monthly = train_db_monthly.melt(
        id_vars=["sys_id", "Year"], value_name="Month"
    )
    del train_db_monthly["variable"]

    train_db_monthly = train_db_monthly.merge(
        train_db, how="left", on=["Year", "sys_id"]
    )
    monthly_skip = model_monthly.feature_status[
        model_monthly.feature_status["monthly Skip"] == 1
    ]["Feature_name"]
    # annual_skip = model_monthly.feature_status[model_monthly.feature_status['Skip'] == 1]['Feature_name']

    features_2_drop = list(set(monthly_skip)) + [
        "wu_rate_backkup",
        "pop_backkup",
    ]
    annual_climate_features = model_monthly.feature_status[
        model_monthly.feature_status["Group_Name"].isin(["Climate"])
    ]["Feature_name"]
    ...
    for var in features_2_drop:
        try:
            ...
            print(
                "Warnining: Unable to remove {} from monthly database".format(
                    var
                )
            )

    # add climate
    monthly_climate.rename(
        columns={"year": "Year", "month": "Month"}, inplace=True
    )
    train_db_monthly = train_db_monthly.merge(
        monthly_climate, how="left", on=["Year", "sys_id", "Month"]
    )
    train_db_monthly.drop_duplicates(
        subset=["sys_id", "Year", "Month"], inplace=True
    )

    # add water _use
    monthly_wu.drop_duplicates(
        subset=["WSA_AGIDF", "YEAR", "Month"], inplace=True
    )
    monthly_wu = monthly_wu[monthly_wu["inWSA"] > 0]
    monthly_wu = monthly_wu[
        (monthly_wu["seasonality_simil_swud"] > 0)
        | (monthly_wu["seasonality_simil_nonswud"] > 0)
    ]

    # let us make sure all nan simil metrics are numeric -- no nans
    monthly_wu.loc[
        monthly_wu["seasonality_simil_swud"].isna(), "seasonality_simil_swud"
    ] = 0
    monthly_wu.loc[
        monthly_wu["seasonality_simil_nonswud"].isna(),
        "seasonality_simil_nonswud",
    ] = 0
    ...
    Good_swud_mask = (
        monthly_wu["seasonality_simil_swud"]
        >= monthly_wu["seasonality_simil_nonswud"]
    )
    Good_nonswud_mask = (
        monthly_wu["seasonality_simil_swud"]
        < monthly_wu["seasonality_simil_nonswud"]
    ...
    monthly_wu.loc[Good_swud_mask, "frac_final"] = monthly_wu[Good_swud_mask][
        "month_frac_swud"
    ]
    monthly_wu.loc[Good_swud_mask, "simil_final"] = monthly_wu[Good_swud_mask][
        "seasonality_simil_swud"
    ]
    monthly_wu.loc[Good_swud_mask, "monthly_wu"] = monthly_wu[Good_swud_mask][
        "monthly_wu_G_swuds"
    ]
    monthly_wu.loc[Good_swud_mask, "isswuds"] = 1

    monthly_wu.loc[Good_nonswud_mask, "frac_final"] = monthly_wu[
        Good_nonswud_mask
    ]["month_frac_nonswud"]
    monthly_wu.loc[Good_nonswud_mask, "simil_final"] = monthly_wu[
        Good_nonswud_mask
    ]["seasonality_simil_nonswud"]
    monthly_wu.loc[Good_nonswud_mask, "monthly_wu"] = monthly_wu[
        Good_nonswud_mask
    ]["monthly_wu_G_nonswuds"]
    ...
    monthly_wu.rename(
        columns={
            "WSA_AGIDF": "sys_id",
            "YEAR": "Year",
            "frac_final": "monthly_fraction",
            "simil_final": "simil_stat",
        },
        inplace=True,
    )

    train_db_monthly = train_db_monthly.merge(
        monthly_wu, how="left", on=["sys_id", "Year", "Month"]
    )

    # add tourism
    temp_tour = tourism_df[
        [
            "WSA_AGIDF",
            "January",
            "February",
    ...
    temp_tour = temp_tour.melt(
        id_vars=["WSA_AGIDF"], value_name="monthly_tourism"
    )
    temp_tour.rename(
        columns={"variable": "Month", "WSA_AGIDF": "sys_id"}, inplace=True
    )
    ...
    train_db_monthly = train_db_monthly.merge(
        temp_tour, how="left", on=["sys_id", "Month"]
    )
...
if add_full_climate:
    ...
    train_db = train_db.merge(
        annual_climate, on=["sys_id", "Year"], how="left"
    )
    train_db.to_csv(annual_training_file, index=False)
...
if fill_lat_lon_gaps:
    ...
    sys_coord_df = geopandas.GeoDataFrame(
        sys_coord_df, geometry="geometry", crs=wsa_shp.crs
    )
    ...
    del train_db["LAT"]
    train_db = train_db.merge(
        sys_coord_df[["WSA_AGIDF", "X", "Y", "LAT", "LONG"]],
        how="left",
        left_on="sys_id",
        right_on="WSA_AGIDF",
    )
...
if add_AWUS_data:
    ...
    wsa_county_map = wsa_county_map.drop_duplicates(
        subset="WSA_AGIDF", keep="first"
    )
    ...
    wsa_county_map = wsa_county_map.merge(
        awuds_df_, how="left", left_on="GEOID", right_on="county_id"
    )
    wsa_county_map.rename(
        columns={
            "PS-WTotl": "awuds_totw_cnt",
            "PS-DelDO": "awuds_dom_cnt",
            "PS-TOPop": "awuds_pop_cnt",
            "WSA_AGIDF": "sys_id",
        },
        inplace=True,
    )
...
if add_building_footprint:
    ...
    blg_ft_df.rename(
        columns={
            "count": "bdg_ftp_count",
            "gt_2median": "bdg_gt_2median",
            "gt_4median": "bdg_gt_4median",
            "lt_2median": "bdg_lt_2median",
            "lt_4median": "bdg_lt_4median",
        },
        inplace=True,
    )
...
if use_fractions_of_income_age:
    ...
    def fix_negative_income(df):
        ...
        for feat in income_cols:
            inc = income_dic[feat]
            df["average_income"] = (
                df["average_income"] + inc * df["prc_" + feat]
            )
...
if Add_Manufacturing_establishments:
    ...
    df["Num_establishments_2012"] = pd.to_numeric(
        df["Num_establishments_2012"], errors="coerce"
    )
    train_db = train_db.merge(
    ...
    del df["Number of establishments"]
    df["Num_establishments_2017"] = pd.to_numeric(
        df["Num_establishments_2017"], errors="coerce"
    )
    train_db = train_db.merge(
        df, left_on="county_ids", right_on="countysss", how="left"
    )
...
if compute_education_perc:
    edu_cols = [
        "n_lt_ninth_gr	n_ninth_to_twelth_gr	n_hs_grad	n_some_college	n_associates	n_bachelors	n_masters_phd"
    ]
...
if add_annual_water_use:
    ...
    def get_annual_wu(model, annual_wu):
        ...
        cleanned_wu["fswuds_pc"] = cleanned_wu["fswud"] / (
            cleanned_wu["days_in_year"] * cleanned_wu["pop"]
        )
        cleanned_wu["fnonswuds_pc"] = cleanned_wu["fnonswud"] / (
            cleanned_wu["days_in_year"] * cleanned_wu["pop"]
        )
        ...
        model.log.to_table(
            cleanned_wu[cols].describe(percentiles=[0.05, 0.5, 0.95, 1]),
            header=-1,
        )
        ...
        monthly_pc = cleanned_wu["annual_wu_from_monthly_swuds_G"] / (
            365 * cleanned_wu["pop"]
        )
        ...
        cleanned_wu.loc[mask, "fswud"] = cleanned_wu.loc[
            mask, "annual_wu_from_monthly_swuds_G"
        ]
        cleanned_wu.loc[mask, "swuds_pc"] = cleanned_wu.loc[mask, "fswud"] / (
            365 * cleanned_wu.loc[mask, "pop"]
        )
        cleanned_wu["fswuds_pc"] = cleanned_wu["fswud"] / (
            cleanned_wu["days_in_year"] * cleanned_wu["pop"]
        )
        ...
        maskpc = (cleanned_wu["fswuds_pc"] < 20) | (
            cleanned_wu["fswuds_pc"] > 600
        )
        cleanned_wu.loc[(mask & maskpc), ["fswud"]] = np.NAN

        mask = cleanned_wu["prc_time_change_nonswud"] > 50
        maskpc = (cleanned_wu["fnonswuds_pc"] < 20) | (
            cleanned_wu["fnonswuds_pc"] > 600
        )
        ...
        swud16_pc = cleanned_wu["fswud"] / (
            cleanned_wu["days_in_year"] * cleanned_wu["pop_swud16"]
        )
        tpopsrv_pc = cleanned_wu["fswud"] / (
            cleanned_wu["days_in_year"] * cleanned_wu["TPOPSRV"]
        )
        ...
        badswud = (
            (cleanned_wu["fswuds_pc"] <= 20)
            | (cleanned_wu["fswuds_pc"] > 500)
            | (cleanned_wu["fnonswuds_pc"] <= 20)
            | (cleanned_wu["fnonswuds_pc"] > 500)
        )
        goodswud16 = (swud16_pc > 20) & (swud16_pc <= 500)
        cleanned_wu.loc[badswud & goodswud16, "pop_enh"] = cleanned_wu.loc[
            badswud & goodswud16, "pop_swud16"
        ]

        cleanned_wu["fswuds_pc"] = cleanned_wu["fswud"] / (
            cleanned_wu["days_in_year"] * cleanned_wu["pop_enh"]
        )
        cleanned_wu["fnonswuds_pc"] = cleanned_wu["fnonswud"] / (
            cleanned_wu["days_in_year"] * cleanned_wu["pop_enh"]
        )
        ...
        cleanned_wu.loc[badswud & goodtpop16, "pop_enh"] = cleanned_wu.loc[
            badswud & goodtpop16, "TPOPSRV"
        ]
        ...
        badswud = (
            (cleanned_wu["fswuds_pc"] <= 20)
            | (cleanned_wu["fswuds_pc"] > 500)
            | cleanned_wu["fswuds_pc"].isna()
        )
        gooNonSWUD = (cleanned_wu["fnonswuds_pc"] > 20) & (
            cleanned_wu["fnonswuds_pc"] <= 500
        )
        cleanned_wu.loc[badswud & gooNonSWUD, "final_wu"] = cleanned_wu.loc[
            badswud & gooNonSWUD, "fnonswud"
        ]
        cleanned_wu["final_wu_pc"] = cleanned_wu["final_wu"] / (
            cleanned_wu["days_in_year"] * cleanned_wu["pop_enh"]
        )
        model.log.to_table(
            cleanned_wu[["final_wu", "final_wu_pc"]].describe(
                percentiles=[0.05, 0.5, 0.95, 1]
            ),
            header=-1,
        )
        ...
        cleanned_wu["final_wu_mgd"] = (
            cleanned_wu["final_wu"] / cleanned_wu["days_in_year"]
        )
    ...
    systems_with_pop_change = annual_wu_ready[
        annual_wu_ready["pop"] - annual_wu_ready["pop_enh"] > 0.1
    ][["WSA_AGIDF", "YEAR", "pop_enh", "pop"]]
    systems_with_pop_change = systems_with_pop_change.drop_duplicates(
        subset=["WSA_AGIDF", "YEAR"]
    )
    systems_with_pop_change.rename(
        columns={"WSA_AGIDF": "sys_id", "YEAR": "Year"}, inplace=True
    )
    train_db["pop_backkup"] = train_db["pop"].copy()
    train_db["wu_rate_backkup"] = train_db["wu_rate"].copy()
    systems_with_pop_change["corr_ratio"] = (
        systems_with_pop_change["pop_enh"] / systems_with_pop_change["pop"]
    )
    systems_with_pop_change = systems_with_pop_change[
        ["sys_id", "Year", "corr_ratio"]
    ]
    train_db = train_db.merge(
        systems_with_pop_change, how="left", on=["sys_id", "Year"]
    )
...
if add_cii_water_use:
    ...
    temp_ = train_db.merge(
        cii_fractions_df[["wsa_agidf", "year", "cii_frac", "dom_frac"]],
        how="left",
        left_on=["sys_id", "Year"],
