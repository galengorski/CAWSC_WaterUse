...
def extract_wu_from_swud(swud_df):
    ...
    month_swud['monthly_flow'] = 1e6 * month_swud['mWU'] * month_swud['days_in_month'] # monthly flow is G/month
    month_swud['monthly_flow'] = month_swud['monthly_flow'].abs()

    annual_df_from_months = month_swud.groupby(by=['WSA_AGIDF', 'YEAR']).sum()
    annual_df_from_months['annual_flow'] = annual_df_from_months['monthly_flow'] # annual flow is G/year
    ...
    ann_ann_df.loc[mask, 'annual_wu_G'] = 1e6 * (ann_ann_df.loc[mask, 'TOT_WD_MGD']).values*366
    ann_ann_df.loc[~mask, 'annual_wu_G'] = 1e6 * (ann_ann_df.loc[~mask, 'TOT_WD_MGD']).values*365
...
def extract_wu_from_nonswud(non_swud_data):
    ...
    monthly_df = non_swud_data.drop(["STATE", 'ANNUAL_MG', 'DATA_FREQ', 'POPULATION',    'PA_FLAG'], axis=1)
    monthly_df.dropna(axis=0, inplace=True)
    monthly_df = monthly_df.melt(id_vars=["WSA_AGIDF", "YEAR"], var_name="Month", value_name="wu_rate_MG")
...
def join_swuds_nonswuds(swuds, nonswuds):
    ...
    monthly_nonswud = monthly_nonswud.merge(annual_nonswud, how='left', on=['WSA_AGIDF', 'YEAR'])
    temp_ = monthly_nonswud[['WSA_AGIDF', 'YEAR', 'annual_wu_G']].copy()
    temp_.drop_duplicates(subset = ['WSA_AGIDF', 'YEAR'], inplace = True)
    annual_nonswud = annual_nonswud.merge(temp_, how='left', on=['WSA_AGIDF', 'YEAR'])
    annual_nonswud.rename(columns={'annual_wu_G_x': 'annual_wu_G', 'annual_wu_G_y': 'annual_wu_from_monthly_G'},
                          inplace=True)


    all_monthly = monthly_swud.merge(monthly_nonswud, how='outer', on=['WSA_AGIDF', 'YEAR', 'Month'])
    all_annual = annual_swud.merge(annual_nonswud, how='outer', on=['WSA_AGIDF', 'YEAR'])

    all_annual.rename(columns = {'annual_wu_G_x':'annual_wu_G_swuds', 'annual_wu_G_y':'annual_wu_G_nonswuds',
                                 'annual_wu_from_monthly_G_x':'annual_wu_from_monthly_swuds_G',
                                 'annual_wu_from_monthly_G_y': 'annual_wu_from_monthly_nonswuds_G',
                                 }, inplace = True)
    all_monthly.rename(columns={'monthly_wu_G_x': 'monthly_wu_G_swuds', 'monthly_wu_G_y': 'monthly_wu_G_nonswuds',
                                'annual_wu_G_x':'annual_wu_G_swuds', 'annual_wu_G_y':'annual_wu_G_nonswuds'},
                      inplace=True)
...
def code_wu_outliers(annual_wu, monthly_wu, master_pop):
    ...
    annual_wu = annual_wu.merge(master_pop[['WSA_AGIDF', 'YEAR', 'pop_swud16', 'pop', 'TPOPSRV']], how='left', on=['WSA_AGIDF', 'YEAR'])
    monthly_wu = monthly_wu.merge(master_pop[['WSA_AGIDF', 'YEAR', 'pop_swud16', 'pop', 'TPOPSRV']], how='left',
                                on=['WSA_AGIDF', 'YEAR'])

    # (1) find swuds systems where monthly data exist but not annual
    mask_1 = (annual_wu['annual_wu_from_monthly_swuds_G'] > 0) & (
                (annual_wu['annual_wu_G_swuds'] == 0) | (annual_wu['annual_wu_G_swuds'].isna()))
    mask_1n = (annual_wu['annual_wu_from_monthly_nonswuds_G'] > 0) & (
                (annual_wu['annual_wu_G_nonswuds'] == 0) | (annual_wu['annual_wu_G_nonswuds'].isna()))
    ...
    rrratio = annual_wu['annual_wu_from_monthly_swuds_G'] / annual_wu['annual_wu_G_swuds']
    ...
    # same but non swud
    rrratio = annual_wu['annual_wu_from_monthly_nonswuds_G'] / annual_wu['annual_wu_G_nonswuds']
    rrratio[rrratio.isin([np.inf, -np.inf])] = np.NAN
    ratio_mask = (rrratio>1.1) | (rrratio<0.90)
    ...
    temp_diff = annual_wu.groupby('WSA_AGIDF').apply(compute_temporal_variability, field = 'annual_wu_G_swuds')
    temp_diff = pd.DataFrame(temp_diff).reset_index().rename(columns={0: 'prc_time_change_swud'})
    annual_wu = annual_wu.merge(temp_diff, on='WSA_AGIDF', how='left')

    temp_diff = annual_wu.groupby('WSA_AGIDF').apply(compute_temporal_variability, field='annual_wu_G_nonswuds')
    temp_diff = pd.DataFrame(temp_diff).reset_index().rename(columns={0: 'prc_time_change_nonswud'})
    annual_wu = annual_wu.merge(temp_diff, on='WSA_AGIDF', how='left')
    ...
    annual_wu['swuds_pc'] = annual_wu['annual_wu_G_swuds'] / (annual_wu['pop'] * annual_wu['days_in_year'])
    annual_wu['swuds_pc'].replace([np.inf, -np.inf], np.nan, inplace=True)
    annual_wu['nonswuds_pc'] = annual_wu['annual_wu_G_nonswuds'] / (annual_wu['pop'] * annual_wu['days_in_year'])
    annual_wu['nonswuds_pc'].replace([np.inf, -np.inf], np.nan, inplace=True)

    # (5) flag inconsistent annual swud vs noswud data
    annual_wu['swud_noswud_diff_prc'] = 100.0*np.abs(annual_wu['annual_wu_G_swuds']-annual_wu['annual_wu_G_nonswuds'])/annual_wu['annual_wu_G_swuds']

    # (6) flag inconsistent pop and TPOPSRV
    annual_wu['pswud16_tpop_ratio'] = 100.0 * np.abs(annual_wu['TPOPSRV'] - annual_wu['pop_swud16']) / \
                                   annual_wu['pop_swud16']
    annual_wu['ppop_tpop_ratio'] = 100.0 * np.abs(annual_wu['TPOPSRV'] - annual_wu['pop']) / \
                                      annual_wu['TPOPSRV']

    # (7) check normality of monthly fractions - swud
    monthly_wu['month_frac_swud'] = monthly_wu['monthly_wu_G_swuds'] / monthly_wu['annual_wu_G_swuds']
    monthly_wu = outliers_utils.flag_monthly_wu_abnormal_fac(monthly_wu, sys_id = 'WSA_AGIDF' ,
                                               year = 'YEAR' , month = 'Month' , mon_wu = 'monthly_wu_G_swuds'
                                               , ann_wu = 'annual_wu_G_swuds')
    monthly_wu.rename(columns={'seasonality_simil': 'seasonality_simil_swud'}, inplace=True)
    # (8) check normality of monthly fractions - nonswud
    monthly_wu['month_frac_nonswud'] = monthly_wu['monthly_wu_G_nonswuds'] / monthly_wu['annual_wu_G_nonswuds']
    monthly_wu = outliers_utils.flag_monthly_wu_abnormal_fac(monthly_wu, sys_id='WSA_AGIDF',
                                                             year='YEAR', month='Month', mon_wu='monthly_wu_G_nonswuds'
                                                             , ann_wu='annual_wu_G_nonswuds')
    monthly_wu.rename(columns={'seasonality_simil': 'seasonality_simil_nonswud'}, inplace=True)

    annual_wu["inWSA"] = 0
    annual_wu.loc[annual_wu['WSA_AGIDF'].isin(wsa_shp['WSA_AGIDF']), "inWSA"] = 1

    monthly_wu['inWSA'] = 0
    monthly_wu.loc[monthly_wu['WSA_AGIDF'].isin(wsa_shp['WSA_AGIDF']), "inWSA"] = 1
...
def resolve_conflicts(annual_wu, monthly_wu, regional_average):
    ...
    regional_average = regional_average[
        ['sys_id', 'x', 'y', 'pop_tmean', 'pop_median', 'swud_tmean', 'swud_median', 'tpop_tmean', 'tpop_median']].copy()
...
if __name__ == "__main__":
    dataset = pd.read_csv(r"../../ml_experiments/annual_v_0_0/clean_train_db.csv")
    ...
    if 1:
        monthly_wu = pd.read_csv("../../ml_experiments/annual_v_0_0/monthly_wu.csv")
        annual_wu = pd.read_csv("../../ml_experiments/annual_v_0_0/annual_wu.csv")
        regional_average = pd.read_csv("../../ml_experiments/annual_v_0_0/spatial_pc_statistics_apply_max_min_pc.csv")
