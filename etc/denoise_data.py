...
def remove_signal(df_noise, features, test_frac=0.3, signal_model=None):
    df_noise['err'] = np.power((signal_model.predict(df_noise[features]) - df_noise['y']), 2)
...
def cross_validate(new_df, features, score):
    ...
    signal_scores = cross_val_score(gb, new_df[features], new_df['y'],
                                    scoring=score, cv=5)
...
def algo(df):
    ...
    signal_scores = cross_val_score(gb, df_signal[features], df_signal['y'],
                                    scoring=score, cv=5)

    noise_scores = cross_val_score(gb, df_noise[features], df_noise['y'],
                                   scoring=score, cv=5)
    ...
    for iter in range(200):
        ...
        new_score, new_df, df_move = propose_sample_removal(df_signal, features, test_frac=0.3, isnoise=0)
        ...
        new_score, new_df, df_move = propose_sample_removal(df_noise, features, test_frac=0.3, isnoise=1)
        ...
        ax2.clear()
        plot4, = ax2.plot(df_noise[features], df_noise['y'], c='r', alpha=0.5, linestyle='None', marker='o')
        plot3, = ax2.plot(df_signal[features], df_signal['y'], c='b', alpha=0.5, linestyle='None', marker='o')
...
def visulize(initialize=True, fig=None, axs=None,
             noise_average_score=None, signal_average_scroe=None,
             df_signal=None, df_noise=None, features=None, iter=1,
             frac_noise_list=None, df_move=None
             ):
    if initialize:
...
def algo_mcmc(df):
    ...
    for iter in range(N):
        ...
        # remove outliers from  signal
        new_score, new_df, df_move, signal_model = propose_sample_removal(df_signal, features, test_frac=0.3, isnoise=0)
        ...
        if 1:
            new_score, new_df, df_move, outlier_model = propose_sample_removal(df_noise, features, test_frac=0.3,
                                                                               isnoise=1)
            frac_of_noise = len(df_noise) / (len(df_signal))
        ...
        visulize(initialize=False, fig=fig, axs=axs,
                 noise_average_score=noise_average_score, signal_average_scroe=signal_average_scroe,
                 df_signal=df_signal, df_noise=df_noise, features=features, iter=iter,
                 frac_noise_list=frac_noise_list, df_move=out_df_mov)
...
def algo_mcmc_official(df, **kwargs):
    ...
    gb = xgb_estimator(params=None)
    signal_scores = cross_val_score(gb, df_signal[features], df_signal[target],
    ...
    for iter in range(N):
        out_df_mov = pd.DataFrame(columns=df_signal.columns)
        # remove outliers from  signal
        new_score, new_df, df_move, signal_model = propose_sample_removal(df_signal, features, test_frac=test_frac,
                                                                          isnoise=0)
        frac_of_noise = len(df_noise) / len(df_signal)
        ...
        if u <= gamma:
            if signal_frac > min_signal_ratio:
                new_score = smoother(signal_average_score[-1], new_score, damping_weight)
                df_noise = pd.concat([df_noise, df_move], axis=0)
                df_signal = new_df
        ...
        frac_noise_list.append(frac_of_noise)
        visulize(initialize=False, fig=fig, axs=axs,
                 noise_average_score=noise_average_score, signal_average_scroe=signal_average_score,
                 df_signal=df_signal, df_noise=df_noise, features=features, iter=iter,
                 frac_noise_list=frac_noise_list, df_move=out_df_mov)
