...
if add_places_pop_to_training:
    wsa_places_shp = geopandas.read_file(
        r"C:\work\water_use\gis_data\WSA_Census_Places_Maping_intersect.shp"
    )
    ...
    agg_pop_df = pd.read_csv(
        os.path.join(
            db_root, r"ml\training\misc_features\WSA_V1_fromCheryl.csv"
        )
    )
    ...
    wsa_places_intersect_density = geopandas.read_file(
        r"C:\work\water_use\gis_data\wsa_census_places_den.shp"
    )

    wsa_pop_from_places = wsa_places_intersect_density.groupby(
        by=["WSA_AGIDF"]
    ).sum()
    ...
    place_pop_ts["complte_fips"] = place_pop_ts[
        ["state_fips", "place_fips"]
    ].agg("".join, axis=1)
    if 1:
        xx = wsa_places_intersect_density.merge(
            place_pop_ts,
            left_on="PLACEFIPS",
            right_on="complte_fips",
            how="left",
        )
        ...
        xx = xx.merge(
            wsa_area, left_on="WSA_AGIDF", right_on="WSA_AGIDF", how="left"
        )
        ...
        xx = xx.merge(
            usgs_pop, left_on="WSA_AGIDF", right_on="WSA_AGIDF", how="left"
        )
...
def filter_out_rapid_pop_changes(_df):
    ...
    ref_df = ref_df[
        ref_df["Year"] < 2020
    ]  # census population for 2020 is not great

    ref_df = ref_df[
        (ref_df["diff"] < ref_df["diff"].quantile(0.9))
...
if fix_pop:
    ...
    place_pop = pd.read_csv(
        r"C:\work\water_use\mldataset\ml\training\misc_features\population_by_pace_census\SUB-EST2020_ALL.csv"
    )
    place_wsa_map = geopandas.read_file(
        r"C:\work\water_use\gis_data\wsa_census_places_den.shp"
    )

    tract_pop = train_db[["sys_id", "Year", "population"]]
    yy = tract_pop.groupby(["sys_id"], group_keys=False).apply(
        filter_out_rapid_pop_changes
    )
    ...
    wsa_pop["pp_ratio"] = (
        np.abs(wsa_pop["POP_SRV"] - (wsa_pop["POLY_POP"])) / wsa_pop["POP_SRV"]
    )
    ...
    bad_train_db = bad_train_db.merge(
        wsa_pop, how="left", left_on="sys_id", right_on="WSA_AGIDF"
    )
