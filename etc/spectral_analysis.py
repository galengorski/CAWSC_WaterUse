...
with open("pa_spectral_graph.html", "w") as fid:
    for ws in wsid:
        ...
        mm, mm_frac = od.get_monthly_frac(
            curr_,
            var_col="QUANTITY",
            date_col="REPORT_DATE",
            month_col="month",
        )
        tmin, tmin_frac = od.get_monthly_frac(
            curr_, var_col="tmmn", date_col="REPORT_DATE", month_col="month"
        )
        ...
        if 1:
            ...
            fig.add_trace(
                go.Scatter(x=(1 / f), y=Pxx_den, mode="lines", name="lines")
            )
            fig.add_trace(
                go.Scatter(
                    x=Mwu.index, y=Mwu["QUANTITY"], mode="lines", name="lines"
                ),
                row=1,
                col=2,
            )
