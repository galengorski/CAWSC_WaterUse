...
cluster_shp = geopandas.read_file(
    r"C:\work\water_use\mldataset\gis\clusters\WSA_Clustering_info.shp"
)
...
exg_sys_merged["swuds_pc"] = np.log10(
    1 + exg_sys_merged["annual_wu_G_swuds"] / (365 * exg_sys_merged["pop"])
)
exg_sys_merged["nonswuds_pc"] = np.log10(
    1 + exg_sys_merged["annual_wu_G_nonswuds"] / (365 * exg_sys_merged["pop"])
)
...
print(
    "Number of unique systems in Nonswud data is {}".format(
        len(annual_nonswud["WSA_AGIDF"].unique())
    )
)
...
print(
    "Number of unique nonswud systems that are not in WSA is {}".format(
        len(outWSA_nonswud["WSA_AGIDF"].unique())
    )
)
...
outWSA_nonswud_connected = outWSA_nonswud[
    outWSA_nonswud["WSA_AGIDF"].isin(clust_sys)
]["WSA_AGIDF"].unique()
cc = 1
