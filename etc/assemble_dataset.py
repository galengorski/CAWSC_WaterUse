...
def get_features(huc2s=[], database_root=""):
    ...
    for huc2 in huc2s:
        ...
        if not (str(huc2) in huc2_folders):
            print(
                "There is no HUC2 = {} in {} folder".format(
                    huc2, database_root
                )
            )
            continue
        ...
        # extract census
        census_folder = os.path.join(
            database_root, os.path.join(str(huc2), "census")
        )
        census_df = aggregate_census_data(
            census_folder,
            census_prefix="cs_wsa_",
            cs_features=cs_features,
            skip_wkey="_yearly",
        )
        census_df.to_csv(
            os.path.join(census_folder, "daily_census_{}.csv".format(huc2))
        )
        ...
        climate_df.to_csv(
            os.path.join(climate_folder, "daily_climate_{}.csv".format(huc2))
        )
        xxx = 1
        # extract wu
        # combine
...
def get_all_annual_db(
    ...
    fn_features = os.path.join(
        database_root, r"..\..\others\census_features_to_use.txt"
    )
    ...
    for huc2 in huc2s:
        ...
        feature_folder = os.path.join(database_root, os.path.join(str(huc2)))
        ...
        # extract census
        census_folder = os.path.join(
            database_root, os.path.join(str(huc2), "census_places")
        )
        # extract climate
        climate_folder = os.path.join(
            database_root, os.path.join(str(huc2), "climate")
        ...
        df_annual = assemble_annual_training_dataset(
            wu_file=wu_file,
            wsa_file=wsa_file,
            year_field="YEAR",
            wu_field=wu_field,
            ...
            func_to_process_sys_name=None,
            to_galon=1e6,
            census_file_prefex="cs_wsa_",
            climate_file_prefix="",
            cs_features=cs_features,
        )
        if len(df_annual) > 0:
            df_annual["HUC2"] = huc2
            df_annual.to_csv(
                os.path.join(assemble_folder, "train_db2_{}.csv".format(huc2))
            )

    # now loop over assemble folders to assemble the final file


def get_all_monthly_db(
    database_root,
    wu_file,
    wsa_file,
    update_train_file=False,
    ...
    for huc2 in huc2s:
        ...
        if not ("assemble" in os.listdir(feature_folder)):
            ...
            fn_train = os.path.join(
                assemble_folder, "train_db_monthly_{}.csv".format(huc2)
            )
            if not (update_train_file):
                continue
        ...
        # ****
        df_monthly = assemble_monthly_training_dataset(
            wu_file=wu_file,
            wsa_file=wsa_file,
            year_field="YEAR",
            wu_field=wu_field,
...
def aggregate_census_data(
    census_folder, census_prefix, cs_features, skip_wkey=""
):
    li = []
    ...
    for var in fields_to_interpolate:
        ...
        try:
            f = interpolate.interp1d(
                x_, y_, fill_value="extrapolate", kind="linear"
            )
        except:
            pass
...
def frac_to_date(frac_date):
    ...
    if yyear % 4 == 0:
        ...
        days = round(365 * days)
    ddate = pd.to_datetime("{}-1-1".format(yyear)) + pd.to_timedelta(
        days, unit="D"
    )
    return ddate


def get_water_system_ids_from_folder(
    folder, prefix=["cs_"], skip_prfix="X_X_X"
):
...
def assemble_annual_training_dataset(
    wu_file,
    wsa_file="",
    year_field="year",
    wu_field="wu",
    ...
    wu_df = pd.read_csv(wu_file)[
        [year_field, sys_id_field, wu_field, "T_LAT", "T_LONG", "is_swud"]
    ]

    sys_ids = set(wsa_shp[shp_sys_id_field]).intersection(
        set(wu_df[sys_id_field])
    )
    wu_df = wu_df[wu_df[sys_id_field].isin(sys_ids)]

    wu_df[wu_field] = wu_df[wu_field] * to_galon
    sys_in_census = get_water_system_ids_from_folder(
        census_folder, prefix=["cs_wsa_", "_yearly"], skip_prfix="XXXX"
    )
    ...
    for sys_id in sys_in_census:
        ...
        if len(curr_non_swud) > 0:
            ...
            curr_wu = curr_wu.groupby(
                by=year_field
            ).mean()  ## in case flow exist in both swud and nonswud

        wu_ = pd.DataFrame(
            columns=curr_wu.columns, index=np.arange(minYear, maxYear + 1)
        )
        ...
        if os.path.isfile(fn_cs):
            ...
            cs_df = extrapolate_census_data(
                cs_df, fields_to_interpolate=cs_features
            )

            set1 = list(
                set(
                    cs_features + ["pop_interpolated"],
                ).difference(cs_df.columns)
            )
        ...
        for var in ["etr", "pr", "tmmn", "tmmx"]:
            ...
            if os.path.isfile(clim_fn):
                ...
                if 1:
                    ...
                    cc_df_coo = cc_df2[
                        cc_df2["month"].isin([10, 11, 12, 1, 2, 3])
                    ]
                    del cc_df_coo["month"]
...
def assemble_monthly_training_dataset(
    wu_file,
    wsa_file="",
    year_field="year",
    wu_field="wu",
    ...
    cols_to_load = (
        [year_field, sys_id_field] + list(mon.values()) + ["is_swud"]
    )
    wsa_shp = gpd.read_file(wsa_file)
    ...
    sys_in_census = get_water_system_ids_from_folder(
        census_folder, prefix=["cs_wsa_"], skip_prfix="_yearly"
    )
    ...
    for sys_id in sys_in_census:
        ...
        yr_xv, mo_yv = np.meshgrid(
            np.arange(minYear, maxYear + 1), np.arange(1, 13)
        )
        yr = yr_xv.flatten()
        mo = mo_yv.flatten()
...
def assemble_training_dataset(
    ...
    for sys_id in sys_ids:
        ...
        if os.path.isfile(fn_cs):
            cs_df = pd.read_csv(fn_cs)[
                ["dyear", "population", "median_income", "pop_density"]
            ]
            cs_df["date"] = cs_df["dyear"].apply(frac_to_date)
            cs_df["sys_id"] = sys_id.lower()
...
if __name__ == "__main__":
    ...
    if test_daily:
        ...
        assemble_training_dataset(
            wu_file,
            date_field="PUMP_DATE",
            wu_field="PUMP_Kgal",
            sys_id_field="PWS_ID",
            output_file=output_file,
            census_climate_folder=os.path.dirname(wu_file),
            to_galon=1000,
        )
    ...
    if test_assemble_all_annual:
        ...
        wsa_file = (
            r"C:\work\water_use\mldataset\gis\wsa\WSA_v2_1_alb83_attrib.shp"
        )
    ...
    if test_annually:
        ...
        census_folder = (
            r"C:\work\water_use\mldataset\ml\training\features\6\census"
        )
        climate_folder = (
            r"C:\work\water_use\mldataset\ml\training\features\6\climate"
        )
        cs_features = [
            "population",
            "households2",
        ...
        assemble_annual_training_dataset(
            wu_file=wu_file,
            wsa_file=wsa_file,
            year_field="YEAR",
            wu_field="TOT_WD_MGD",
    ...
    if False:
        ...
        assemble_monthly_training_dataset(
            wu_file=wu_file,
            wsa_file=wsa_file,
            year_field="YEAR",
            wu_field="TOT_WD_MGD",
