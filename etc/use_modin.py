...
if __name__ == "__main__":
    ...
    wu_df = pd.read_csv(
        r"C:\work\water_use\dataset\dailywu\pa\all_PA_raw_data.csv"
    )
    ...
    def remove_outlier_sys(wu_df):
        ...
        for i, sid in enumerate(sys_ids):
            ...
            try:
                ...
                continue
            curr_df["QUANTITY"] = (
                curr_df["QUANTITY"] / curr_df["QUANTITY"].sum()
            )
        ...
        montly.to_csv(
            "C:\work\water_use\dataset\dailywu\pa\pa_monthly_fractions.csv"
        )

    def remove_extreme_values():
        df_fr = pd.read_csv(
            r"C:\work\water_use\dataset\dailywu\pa\pa_monthly_fractions.csv"
        )
        ...
        for col in df_fr.columns:
            if np.any(df_fr[col] > q95) or np.any(df_fr[col] < q5):
                df_fr.drop(col, inplace=True, axis=1)
        df_fr.to_csv(
            r"C:\work\water_use\dataset\dailywu\pa\wu_clean_outliers.csv"
        )
