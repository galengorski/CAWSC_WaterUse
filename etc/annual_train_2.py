...
if __name__ == "__main__":
    ...
    df_feat = df_feat[
        (df_feat["pc_swud_corrected"] > 20)
        & (df_feat["pc_swud_corrected"] < 700)
    ]
    df_feat["ICITE"] = df_feat["awuds_totw_cnt"] - df_feat["awuds_dom_cnt"]
    df_feat["pub_serv_ratio"] = (
        df_feat["awuds_pop_cnt"] / df_feat["county_tot_pop_2010"]
    )
    ...
    if 1:
        df_feat["wu_per_capita"] = df_feat["wu_rate"] / (
            df_feat["LUpop_Swudpop"]
        )
    ...
    if 0:
        ...
        regr = RandomForestRegressor(
            max_depth=4, random_state=0, n_estimators=5000, n_jobs=-1
        )
    ...
    if use_normal_xgb:
        ...
        accuracy = r2_score(
            10**y_test, 10**ypredict, sample_weight=w2_test
        )
        ...
        plt.figure()
        plot_importance(
            gb, xlabel="Gain", importance_type="gain", max_num_features=20
        )

        gain_imp = gb.get_booster().get_score(importance_type="gain")
        gain_imp = pd.DataFrame(gain_imp.items()).sort_values(
            by=[1], ascending=True
        )
