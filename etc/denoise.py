...
def propose_sample_removal(df_, features, target, test_frac=0.3, outliers_sampling_method='equal_weight',
                           error_quantile=0.70, frac_noisy_samples=0.05, kfolds=5,
...
def purify(df, **kwargs):
    ...
    estimator = _get_args('damping_weight', default=xgb_estimator(params=None), **kwargs)
    ...
    for iter in range(N):
        ...
        new_score, new_df, df_move, signal_model = propose_sample_removal(df_signal, features, target,
                                                                          test_frac=test_frac)
        ...
        signal_scores = cross_val_score(estimator, new_df[features], new_df[target],
                                        scoring=score, cv=kfolds)
        new_score = np.mean(signal_scores)
