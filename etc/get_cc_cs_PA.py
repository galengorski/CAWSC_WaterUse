...
if __name__ == '__main__':
    ...
    dc = data_collector.DataCollector(service_area_shapefile=shp_file, apikey=apikey, fieldName=fieldname, output_folder=outws)
    import  time
    start = time.time()
    dc.get_training_data_for_shapefile(thread_num=20,climateFilter=['pet', 'pr', 'tmmn', 'tmmx'], years = [1990, 2000,2001,2003] + list(range(2004, 2020)) )
    endd = time.time()
